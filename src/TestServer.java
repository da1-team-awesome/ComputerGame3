...
public class TestServer {
    ...
    public static String test(String exercise) throws IOException {
        ...
        if(auID.isEmpty() || code.isEmpty()){
            ...
            System.out.print("FÃ¸rste gang du uploader et projekt til testserveren skal du indtaste dit auID og en adgangskode.\n" +
                "Begge dele huskes i din projektmappe, sÃ¥ledes at du ikke behÃ¸ver at indtaste dem ved senere uploads af projektet.\n\n" +
    ...
    private static void downloadCourseFiles(String version, String... files)  {
        for(String file : files)
            try {
                downloadFile("https://tildeweb.au.dk/au531793/dintprog/e20/opgaver/"+version+"/"+file,file);
            } catch(IOException e){
                System.err.println("Unable to download file: "+file+". Reason: "+e.getMessage());
            }
            
        JOptionPane.showMessageDialog(null, "Download af filer for version '"+version+"' blev gennemfÃ¸rt succesfuldt. Det kan vÃ¦re nÃ¸dvendigt at genstarte BlueJ for at se de nye filer.", "Filer downloadet succesfuldt.", JOptionPane.INFORMATION_MESSAGE);
    }
